...
// Objeto para rastrear el estado de visibilidad de cada elemento
...
document.addEventListener("scroll", () => {
  animatedElements.forEach((element) => {
    ...
    const isAlreadyInView = elementStates.get(element) || false;

    if (isVisible && !isAlreadyInView) {
      // Si el elemento entra en vista por primera vez
      const animationClass = element.dataset.animation; // Obtener la clase de animación
      element.classList.add(animationClass);

      // Guardar que el elemento ahora está visible
      elementStates.set(element, true);

      // Eliminar la clase al terminar la animación para que pueda aplicarse nuevamente
    ...
    } else if (!isVisible && isAlreadyInView) {
      // Si el elemento sale del viewport, reiniciar su estado
